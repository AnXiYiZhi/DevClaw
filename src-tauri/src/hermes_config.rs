...
///
/// Input (from DevClaw UI / database):
/// ```json
...
/// `providers:` dict. DevClaw treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by DevClaw)
...
/// keeping DevClaw free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay DevClaw must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a DevClaw edit to an unrelated field would silently
...
// DevClaw fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later DevClaw edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a DevClaw toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
